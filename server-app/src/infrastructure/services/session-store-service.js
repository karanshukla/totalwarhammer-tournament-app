 ...
 * Only the store implementation actually selected is imported, so the unused
 * one's dependency tree never loads into the process.
 * @param {Object} options - Configuration options
 * @returns {Promise<Object>} Configured session store (Redis or MongoDB)
 */
export async function configureSessionStore() {
  let sessionStore;

  if (process.env.USE_MONGO_SESSION === "true" || !process.env.REDIS_URL) {
    logger.info("Using MongoDB session store");
    const { default: MongoDBStore } = await import("connect-mongodb-session");
    ...
    // eslint is complaining about the import below but it is correct
 ...
 * @returns {Promise<Function>} Configured session middleware
 */
export async function configureSessionMiddleware(sessionSecret, isProduction) {
  const sessionStore = await configureSessionStore();
