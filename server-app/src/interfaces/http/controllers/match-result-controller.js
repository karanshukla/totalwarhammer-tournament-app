...
/**
 * Validates that `winnerId` is one of the match's two players and returns the
 * corresponding loser id, or null if `winnerId` doesn't match either player.
 */
function determineLoserId(match, winnerId) {
  ...
  return winnerIdStr === player1Id ? player2Id : player1Id;
}

function getTournamentCreatorId(tournament) {
  return (tournament.createdBy?._id ?? tournament.createdBy)?.toString();
}

async function persistMatch(match, { invalidateStats = true } = {}) {
  await match.save();
  if (invalidateStats) invalidateStatsCache().catch(() => {});
...
async function reportResultOnce(req, res) {
  ...
  const nameMatch = (candidateName) => {
    ...
    return !!normalizedUserName && normalizedCandidate === normalizedUserName;
  };
  ...
  await persistMatch(match, { invalidateStats: match.status === "completed" });
...
export const resolveDispute = async (req, res) => {
  try {
    ...
    const createdById = getTournamentCreatorId(match.tournament);
    const isCreator = createdById === req.user.id?.toString();
    logger.debug(
      `resolveDispute auth: createdById=${createdById} req.user.id=${req.user.id} match=${isCreator}`,
    );
    if (!isCreator) {
      return res.status(403).json({
        success: false,
    ...
    logger.info(
      `Dispute resolved on match ${match._id} by user ${req.user.id}: winner=${winnerId?.toString()}, reason="${reason || "none"}"`,
    );
    return res.status(200).json({ success: true, data: match });
  } catch (error) {
...
export const recordResult = async (req, res) => {
  try {
    ...
    logger.info(
      `Match ${match._id} result recorded by admin ${req.user.id}: winner=${winnerId?.toString()}`,
    );
    return res.status(200).json({ success: true, data: match });
  } catch (error) {
...
export const overrideResult = async (req, res) => {
  try {
    ...
    const createdById = getTournamentCreatorId(match.tournament);
    const isAdmin = createdById === req.user.id?.toString();
    logger.debug(
      `overrideResult auth: createdById=${createdById} req.user.id=${req.user.id} match=${isAdmin}`,
    );
    if (!isAdmin) {
      return res.status(403).json({
        success: false,
    ...
    logger.warn(
      `Match ${match._id} result overridden by admin ${req.user.id}: ${previousWinner} → ${winnerId?.toString()}, reason="${reason || "none"}"`,
    );
    return res.status(200).json({ success: true, data: match });
  } catch (error) {
...
export const updateMatchStatus = async (req, res) => {
  try {
    ...
    await persistMatch(match, { invalidateStats: wasCompleted });
