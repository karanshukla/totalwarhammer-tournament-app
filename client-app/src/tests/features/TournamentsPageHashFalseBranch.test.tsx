 ...
 *
 * False branch: invalid hash fired while already on tabs[0] (createTournament).
...
describe("TournamentsPage – handleHashChange line 143 false branch", () => {
  beforeEach(() => {
    // Reset hash to ensure we start on first tab (createTournament)
  ...
  it("does nothing when invalid hash fires and activeTab is already tabs[0] (line 143 false branch)", async () => {
    ...
    // Page starts on first tab (createTournament) — activeTab === tabs[0].id
    expect(screen.queryByText("Tournament Participants")).toBeNull();

    // Fire hashchange with an invalid tab id while already on the first tab
    await act(async () => {
  ...
  it("resets to first tab when invalid hash fires and activeTab is NOT tabs[0] (line 143 true branch)", async () => {
    ...
    // Now activeTab is brackets (not tabs[0])
